...
class Augmented_WGAN_GP:
    ...
    tdDeConv = lambda **kwds: tf.keras.layers.TimeDistributed(**kwds)
    ...
    def build_generator(self):
        ...
        x = tf.keras.layers.TimeDistributed(ops.UpConv2D(dim//2 * mult, 4, 1, 'valid')(x))
        x = ops.BatchNorm()(x)
        x = layers.ReLU()(x)

        while mult > 1:
            x = tf.keras.layers.TimeDistributed(ops.UpConv2D(dim//2 * (mult // 2))(x))
            x = ops.BatchNorm()(x)
            x = layers.ReLU()(x)

            mult //= 2

        x = tf.keras.layers.TimeDistributed(ops.UpConv2D(3)(x))
    ...
    def build_discriminator(self):
        ...
        x = tf.keras.layers.ConvLSTM2D(dim//2)(x)
        x = ops.LeakyRelu()(x)

        while i > 4:
            x = tf.keras.layers.ConvLSTM2D(dim//2 * (2 * mult))(x)
        ...
        x = tf.keras.layers.ConvLSTM2D(1, 4, 1, 'valid')(x)
